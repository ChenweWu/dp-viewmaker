...
def main(config):
    ...
    for epoch in range(NUM_EPOCH):
        ...
        y_list = []
        pred_list = []
        for X, y,fn in tqdm(train_loader,total=len(train_loader)):
            optimizer.zero_grad()
            X = X.float().to(DEVICE)
            y = y.long().to(DEVICE)
            X = view_model(X)
...
#                     loss += criterion(pred[:,i], y[:,i])
        ...
        with open(config['path']+"train_metrics.txt", 'a') as f:
            f.write("")
    ...
    #                     loss += criterion(pred[:,i], y[:,i])
        ...
        if (epoch+1) % config['chkpt_step'] == 0:
            torch.save(state, config['path'] +config['model']+' epoch '+str(epoch)+"f1 {}".format(f1)+"fullVMK")
            # torch.save(model.state_dict(), config['model']+"k_" + str(curr_fold) + ".pth")
            print("saved...")
        with open(config['path']+"metrics_neighbro.txt", 'a') as f:
            f.write("")
