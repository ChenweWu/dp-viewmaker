...
class MultiClassFocalLoss(nn.Module):
    def __init__(self, embs1, labels, alpha, gamma=2, reduction='mean'):
        ...
        self.target=labels
    def get_loss(self):
        alpha = self.alpha[self.target.long()]  # 为当前batch内的样本，逐个分配类别权重，shape=(bs), 一维向量
        log_softmax = torch.log_softmax(self.pred, dim=1) # 对模型裸输出做softmax再取log, shape=(bs, 3)
        logpt = torch.gather(log_softmax, dim=1, index=self.target.long().view(-1, 1))  # 取出每个样本在类别标签位置的log_softmax值, shape=(bs, 1)
